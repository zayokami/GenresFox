...
7. **Rename the output file** (optional)
   ...
   - You can rename it to `GenresFox-v0.4.8.0.crx` or similar
...
.\package.ps1 -OutputName "GenresFox-v0.4.8.0.crx"
```
...
# Or specify custom output name
./package.sh GenresFox-v0.4.8.0.crx
```
...
crx pack src -o GenresFox-v0.4.8.0.crx
```
...
7. **重命名输出文件**（可选）
   - 默认名称是 `src.crx`
   - 您可以将其重命名为 `GenresFox-v0.4.8.0.crx` 或类似名称
